...
const TitanNearbyScreen = ({ onBack }: { onBack: () => void }) => {
  ...
  // Realtime chat state with the cashier
  ...
  useEffect(() => {
    ...
    // Setup active real-time updates: Realtime merchant balances ticking and distance subtle drift
    liveUpdateIntervalRef.current = setInterval(() => {
      setMerchants(prev => 
        prev.map(m => {
          ...
          // Subtle distance drift representing live walking / movement
        ...
        <div className="px-5 pt-6 pb-6 space-y-6">
          ...
          <div className="relative aspect-video w-full rounded-[24px] bg-slate-950/95 dark:bg-black overflow-hidden border border-border/20 flex flex-col items-center justify-center">
            ...
            {merchants.map((merchant, idx) => {
              // Calculate deterministic display offset based on their lat/lng offsets
            ...
            <div className="space-y-2">
              {merchants.map((merchant) => (
                <div 
                  key={merchant.id}
                  onClick={() => handleOpenTunnel(merchant)}
                  className="ios-list-group p-4 bg-card border border-border/20 hover:border-border/60 transition-all flex flex-col gap-3 active:scale-[0.99] cursor-pointer"
                >
                    ...
                    <div className="space-y-0.5">
                      <h4 className="text-sm font-bold text-foreground flex items-center gap-1.5">
                        ...
                        <span className="flex items-center text-[10px] font-bold text-amber-500">
                          <Star size={10} fill="currentColor" stroke="none" className="mr-0.5" />
                          {merchant.rating}
                        </span>
                      </h4>
                      <p className="text-[11px] text-muted-foreground flex items-center gap-1 select-none">
                        <MapPin size={10} className="text-muted-foreground/60 shrink-0" /> 
                        <span className="truncate max-w-[140px]">{merchant.address}</span>
                      </p>
                    </div>
                    <span className="text-[8px] font-black tracking-widest bg-emerald-500/10 text-emerald-500 px-2 py-0.5 rounded-full uppercase shrink-0">
                      {merchant.distance}m
                    </span>
                  </div>

                  <div className="flex justify-between items-center pt-2 border-t border-border/10 text-[10px]">
                    <div className="space-y-0">
                      <span className="text-[8px] uppercase tracking-wider text-muted-foreground font-bold">CASH VAULT</span>
                    ...
                    <button 
                      ...
                      className="bg-indigo-500 text-white px-4 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1.5 transition-colors"
                    >
                      <Smartphone size={12} /> Connect Tunnel
                    </button>
                  </div>
                </div>
              ))}
