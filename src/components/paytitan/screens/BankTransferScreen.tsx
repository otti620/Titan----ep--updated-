...
import { ArrowLeft, Building2, Search, CheckCircle2, ArrowRight, Loader2, XCircle, Landmark, CreditCard, Camera, RefreshCw } from 'lucide-react';
...
const BANKS = [
  ...
  { id: '33', name: 'PocketApp by PiggyVest', code: '100033', domain: 'pocketapp.com' },
...
const BankTransferScreen = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className="h-full w-full bg-background flex flex-col relative">
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-5 space-y-6 pt-2">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center pl-1 pt-4">
                  <label className="caption-1 text-muted-foreground font-semibold uppercase tracking-widest">Account Number</label>
                  <button 
                    onClick={() => { hapticFeedback('light'); setShowScanner(true); }}
                    className="flex items-center gap-1.5 text-xs font-bold text-indigo-500 uppercase tracking-wider active:opacity-60 bg-indigo-500/10 px-3 py-1.5 rounded-full"
                  >
                    <Camera size={14} /> Scan Written
                  </button>
                </div>
      ...
      {showScanner && (
        <AccountOcrScanner 
          onScanComplete={(scannedNum, scannedBank) => {
            setAccountNumber(scannedNum);
            if (scannedBank) {
              const matched = BANKS.find(b => 
                b.name.toLowerCase().includes(scannedBank.toLowerCase()) ||
                scannedBank.toLowerCase().includes(b.name.toLowerCase())
              );
              if (matched) {
                setSelectedBank(matched);
                toast.success(`Selected ${matched.name}`);
              }
            }
...
const AccountOcrScanner = ({ onScanComplete, onClose }: { onScanComplete: (acct: string, bankName?: string) => void, onClose: () => void }) => {
  ...
  useEffect(() => {
    async function startCamera() {
      try {
        ...
        setError("Camera access denied or unavailable. Please enter details manually.");
      }
    }
    startCamera();
  ...
  const captureFrame = async () => {
    ...
    try {
      ...
      if (ctx) {
        ...
        toast.loading("Gemini AI Reading Account...", { id: "ocr-scan" });
        ...
        if (res.ok && data.success && data.accountNumber) {
          toast.success("Account Number Scanned!", { id: "ocr-scan" });
          hapticFeedback('success');
          onScanComplete(data.accountNumber, data.bankName);
        } else {
          toast.error(data.message || "Could not read digits. Try placing written text closer.", { id: "ocr-scan" });
        }
      }
    } catch (err) {
      console.error(err);
      toast.error("Scanning failed. Please write manually.", { id: "ocr-scan" });
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black z-[1000] flex flex-col items-center justify-between text-white p-6 font-sans">
      <div className="w-full flex justify-between items-center mt-4">
        <button onClick={onClose} className="text-[17px] font-semibold text-indigo-400 active:opacity-60">
          Cancel
        </button>
        <span className="headline text-[17px] font-bold text-white">Scan Account Number</span>
        <div className="w-10"></div>
      </div>

      <div className="relative w-full aspect-[4/3] max-w-sm rounded-[32px] overflow-hidden border border-white/10 bg-neutral-900 shadow-2xl flex items-center justify-center">
        {error ? (
          <p className="text-center text-sm text-neutral-400 px-6">{error}</p>
        ) : (
          <>
            <video 
              ref={videoRef} 
            ...
            />
            <div className="absolute inset-0 pointer-events-none border-[3px] border-indigo-500 rounded-[32px] m-6 flex flex-col items-center justify-between py-6">
              <span className="text-[11px] font-black uppercase tracking-widest text-indigo-400 bg-black/60 px-3 py-1 rounded-full">
                Written Account Target
              </span>
              <span className="text-[10px] font-medium text-white/60 bg-black/60 px-4 py-1.5 rounded-full text-center max-w-[80%]">
                Place paper/written account inside the frame
              </span>
            </div>
            {isProcessing && (
      ...
      <div className="w-full max-w-xs flex flex-col items-center gap-4 mb-10">
        <button 
          onClick={captureFrame}
          disabled={!!error || isProcessing}
          className="w-20 h-20 bg-white active:scale-95 disabled:opacity-40 hover:scale-105 transition-all rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(255,255,255,0.4)] relative"
        >
        ...
        <p className="text-xs text-neutral-400 text-center font-medium leading-snug">
          Click white button to snap photo of written text. Gemini AI reads it instantly.
        </p>
      </div>
