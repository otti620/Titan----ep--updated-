...
export default function FintechPartnersHub() {
  ...
  const [simPartner, setSimPartner] = useState<'paystack' | 'flutterwave' | 'titan_core' | 'payscribe'>('paystack');
      ...
      case 'titan_core':
        return {
          ...
          eventData: {
            ...
            transactionReference: `TITAN-${simRef}`,
      ...
      <div className="bg-card border border-border rounded-[24px] p-5 flex flex-col gap-4">
        ...
        <div className="p-4 bg-muted/50 rounded-2xl space-y-2">
          ...
          <div className="space-y-1.5 pt-1">
            ...
            <EndpointRow label="TITAN CORE CALLBACK" path="/api/webhooks/titan_core" />
      ...
      {activeSubTab === 'sim' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {(['paystack', 'flutterwave', 'titan_core', 'payscribe'] as const).map((p) => (
              <button
                key={p}
        ...
        <div className="bg-card border border-border shadow-sm rounded-[24px] p-6 space-y-6">
          ...
          <div className="space-y-4">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between pl-1">
                    <label className="text-[10px] uppercase font-bold text-muted-foreground">TITAN CORE SECRET KEY</label>
                    <span className="text-[8px] font-mono text-gray-400">MK_TEST...</span>
                  </div>
                  <input 
                    type="password"
                    value={creds.titanCoreSecretKey}
                    onChange={(e) => saveCreds({ ...creds, titanCoreSecretKey: e.target.value })}
                    className="w-full bg-black/5 dark:bg-white/5 border border-border rounded-xl px-3 py-2.5 text-xs focus:ring-1 focus:ring-indigo-500"
                    placeholder="Enter Titan Core Secret"
                  />
                </div>
