...
const ShortcutItem = ({ label, icon, onClick, badge, subLabel }: { label: string, icon: React.ReactNode, onClick: () => void, badge?: string, subLabel?: string }) => (
  <button 
    onClick={() => { hapticFeedback('light'); onClick(); }}
  ...
  >
    {badge && (
      <div className="absolute -top-1.5 -right-1.5 z-10 bg-red-500 text-white caption-2 font-semibold px-2 py-0.5 rounded-full shadow-sm">
        {badge}
      </div>
    )}
    <div className="w-[60px] h-[60px] rounded-[18px] bg-card border border-border flex items-center justify-center shadow-sm">
