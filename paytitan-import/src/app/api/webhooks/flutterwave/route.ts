...
export async function POST(req: NextRequest) {
  try {
    ...
    // Flutterwave success webhook event
    if (event === "charge.completed" || body.status === "successful") {
      const data = body.data || body;
      const amount = parseFloat(data.amount);
      const email = data.customer?.email;
      const ref = data.tx_ref || data.id?.toString();

      if (!email || isNaN(amount) || amount <= 0) {
        return NextResponse.json({ success: false, message: "Incomplete payment data in Flutterwave payload" }, { status: 400 });
      }
      ...
      if (profileErr || !profile) {
        console.warn(`Flutterwave webhook received for unregistered customer email: ${email}`);
        return NextResponse.json({ success: false, message: "No registered PayTitan account found" }, { status: 404 });
      }

      // 2. Prevent double-crediting by checking if transaction reference has been logged
      ...
      await supabaseAdmin.from('transactions').insert([{
        ...
        description: `Direct Bank Transfer auto-reconciled via Flutterwave VA. Ref: ${ref}`,
        amount: amount,
        status: 'SUCCESS'
      }]);
      ...
      await supabaseAdmin.from('user_notifications').insert([{
        user_id: profile.id,
        type: 'success',
        message: `₦${amount.toLocaleString()} received via Flutterwave Virtual Account!`,
        is_read: false
      }]);
